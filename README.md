...
> Matt fleet extension: see [`docs/FLEET_CHAT_ARCHIVE.md`](docs/FLEET_CHAT_ARCHIVE.md) for the four-harness, content-addressed archive pipeline and [`docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md`](docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md) for the current rollout truth. Reviewed release `3c732d7` is deployed to the reachable New MacBook, Mac Studio, and Mac mini; the verification receipt records 263/263 tests. The repo-root deployment lease and the current docs are on this branch. The Matt-ratified structural addendum is local at `9dd6bfe` and was published/read back through the GitHub connector at `86b505319b9fd30601773fd362d0af4fa704fa38` (tree `78478c927b63d01d32d558f01a6639cf1a5e45cf`). New's first supervised scan completed with Drive blocked, Studio's current launchd scan remains in-flight after a prior `RunFailure`, and Old's retry queue is enabled under launchd but still offline. The Mini remains disk-gated, and runtime File Provider Drive publication is still blocked even though the connector has a verified private `AI Chat Archive` folder with two body-free receipt docs. It adds OpenClaw and Hermes support, local credential redaction, recurring launchd jobs, host isolation, receipts, and a Google Drive publication gate. Do not use `extract_all.sh` for that pipeline.
