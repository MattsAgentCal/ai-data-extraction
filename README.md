...
> Matt fleet extension: see [`docs/FLEET_CHAT_ARCHIVE.md`](docs/FLEET_CHAT_ARCHIVE.md) for the four-harness, content-addressed archive pipeline and [`docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md`](docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md) for the current rollout truth. Reviewed release `3c732d7` is deployed to the reachable New MacBook, Mac Studio, and Mac mini; the verification receipt records 263/263 tests. The deployment lease and docs are published at commit `5cd62da`; New and Studio now have the production six-hour launchd labels loaded and their first live scans are in-flight. The Old MacBook remains offline, Mini remains disk-gated, and the runtime's File Provider Drive publication is still blocked even though the connector has a verified private `AI Chat Archive` folder and body-free receipt doc. It adds OpenClaw and Hermes support, local credential redaction, recurring launchd jobs, host isolation, receipts, and a Google Drive publication gate. Do not use `extract_all.sh` for that pipeline.
