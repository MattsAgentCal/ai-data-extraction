...
> Matt fleet extension: see [`docs/FLEET_CHAT_ARCHIVE.md`](docs/FLEET_CHAT_ARCHIVE.md) for the four-harness, content-addressed archive pipeline and [`docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md`](docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md) for the current rollout truth. Reviewed release `3c732d7` is deployed to the reachable New MacBook, Mac Studio, and Mac mini; the verification receipt records 263/263 tests. The deployment lease and docs are locally at `ed5d2b3` and the preceding tree is published through the GitHub connector at `c39c65a3`; New's first supervised scan completed with Drive blocked, while Studio's current launchd scan remains in-flight after a prior `RunFailure`. The Old MacBook remains offline, Mini remains disk-gated, and the runtime's File Provider Drive publication is still blocked even though the connector has a verified private `AI Chat Archive` folder with two body-free receipt docs. It adds OpenClaw and Hermes support, local credential redaction, recurring launchd jobs, host isolation, receipts, and a Google Drive publication gate. Do not use `extract_all.sh` for that pipeline.
