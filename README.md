...
> Matt fleet extension: see [`docs/FLEET_CHAT_ARCHIVE.md`](docs/FLEET_CHAT_ARCHIVE.md) for the four-harness, content-addressed archive pipeline and [`docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md`](docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md) for the current rollout truth. Reviewed release `3c732d7` is deployed to the reachable New MacBook, Mac Studio, and Mac mini; the verification receipt records 263/263 tests. The repo-root deployment lease and the current docs are on this branch. The Matt-ratified structural handoff amendment is local at `9b747927b6217287e035eecbdee8e6309a9e7f4d` and is published on the owned fork at `56e8b7f5f8a7ad47acd36bcd0a901e95339d4f20`. New's second supervised refresh completed with zero errors and its current goal-session object was published once through the authenticated connector as redacted text (`18kklPXiMM2bzF1ZU8tCzlJJ9k-HblbC_`); Studio's launchd scan remains in-flight after a prior `RunFailure`, and Old's retry queue is enabled under launchd but still offline. The Mini remains disk-gated, runtime File Provider Drive publication and the automatic six-hour proof are still blocked, and the Drive folder now contains two receipt Docs plus two verified redacted text canaries. It adds OpenClaw and Hermes support, local credential redaction, recurring launchd jobs, host isolation, receipts, and a Google Drive publication gate. Do not use `extract_all.sh` for that pipeline.
