...
> Matt fleet extension: see [`docs/FLEET_CHAT_ARCHIVE.md`](docs/FLEET_CHAT_ARCHIVE.md) for the four-harness, content-addressed archive pipeline and [`docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md`](docs/FLEET_CHAT_ARCHIVE_LIVE_STATE.md) for current rollout truth. Reviewed release `3c732d7` is deployed to reachable New MacBook, Mac Studio, and Mac mini; the verification receipt records 263/263 tests. New's launchd receipts are 6:19:36 apart (`runs=2→3`, exit 0), and its seven newly staged redacted objects were imported and metadata-verified in the authenticated Drive folder `AI Chat Archive`. Studio's completed launchd-owned receipt `20260830T040218.778731Z-c65bac22` has zero errors and 78 finalized objects (16 Claude + 62 Codex); the Drive plugin imported all 78 with exact metadata, including the one allowed three-file repair, bringing the folder to 90 items and 418,815,569 Studio bytes. The final docs ref is local `14d4ea3ab3cb2071fc6001ce37b60821abd643c2`, published by plugin as fork commit `819e385306cb3c56099662f27dd55d7ddee3b247` (tree `63a421df9576483967a557605a80692166eccd3f`, parent `419df3dbe390fe1a10e4b5171ae8661f9d7c52d2`, `force=false`). Runtime automatic launchd-to-Drive publication remains unproven because Studio has no File Provider and the plugin is not a launchd executable. Mini remains paused pending Matt's closed-log approval, and Old's launchd retry queue remains enabled/offline. The plugin route supersedes the Desktop DMG; do not use `extract_all.sh` for this pipeline.
